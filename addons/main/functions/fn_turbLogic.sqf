...
private _gustSpeed = [HELICOPTER_TURBULENCE_MIN_TURBULENCE, _maxWindSpeed, random(1)] call BIS_fnc_lerp;
...
if (!isGamePaused && !difficultyEnabledRTD && isEngineOn _vehicle && currentPilot _vehicle == _currentUnit) then {    
...
[{
    ...
    if (_currentUnit in _vehicle) then {
        [_vehicle, _dimensions, _surfaceArea, _maxSpeed, _oldForce, _oldCentre] call Helicopter_Turbulence_fnc_turbLogic;
    };
