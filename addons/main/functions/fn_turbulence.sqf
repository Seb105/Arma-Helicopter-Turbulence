...
/*
 * Author: Seb
 * Helicopter_Turbulence_fnc_turbulence adds dynamic turbulence based on weather (overcast and windstrenght) to any vehicle (although it is designed for air vehicles).
...
[{
	...
	if (vehicle _currentUnit == _vehicle) then {
		...
		if (currentPilot _vehicle == _currentUnit && _vehicle getVariable "HELICOPTER_TURBULENCE_READY") then {
			[_vehicle, _dimensions, _surfaceArea, _maxSpeed] call Helicopter_Turbulence_fnc_turbLogic;
		};
	} else {
